"""
AgroTrust AI – Microsserviço HTTP do Agente ESG.

Expõe o ESGAuditorAgent como API REST autocontida (deployável em K8s).

Endpoints:
  POST /run     – exige scope agent:esg:run; executa o agente sobre ESGInput.
...
_REQUIRED_SCOPE = Scope.AGENT_ESG_RUN
_SERVICE_NAME = "agent-esg"
_DEFAULT_PORT = 8010


def _require_esg_scope(request: Request) -> ServiceIdentity:
    """Depends() para POST /run: exige scope agent:esg:run."""
...
async def lifespan(app: FastAPI) -> AsyncIterator[None]:
    ...
    # Agente compilado uma vez (grafo LangGraph cacheado entre requests)
    app.state.agent = ESGAuditorAgent()
...
def create_app() -> FastAPI:
    ...
    app = FastAPI(
        title="AgroTrust AI – Agent ESG",
        description="Microsserviço HTTP do agente de conformidade ambiental (ESG).",
        version="0.1.0",
        lifespan=lifespan,
    ...
    # Ordem: auth depois de correlation_id (que injeta o id em ctx structlog)
    ...
    @app.post(
        "/run",
        response_model=ESGOutput,
        tags=["agent"],
        dependencies=[Depends(_require_esg_scope)],
    )
    async def run(input_data: ESGInput, request: Request) -> ESGOutput:
        log = logger.bind(
            correlation_id=getattr(request.state, "correlation_id", ""),
            dossie_id=input_data.dossie_id,
        )
        log.info("agent_esg_run_received")
        try:
            agent: ESGAuditorAgent = request.app.state.agent
            output = await agent.run(input_data)
            log.info("agent_esg_run_completed", compliance=output.compliance_status.value)
            return output
        except AgentExecutionError as exc:
            log.error(
                "agent_esg_run_failed",
