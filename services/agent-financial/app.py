"""
AgroTrust AI – Microsserviço HTTP do Agente Financeiro.

Expõe o FinancialAnalystAgent como API REST autocontida.

Endpoints:
  POST /run     – exige scope agent:financial:run; executa Trust Score+ sobre FinancialInput.
...
_REQUIRED_SCOPE = Scope.AGENT_FINANCIAL_RUN
_SERVICE_NAME = "agent-financial"
_DEFAULT_PORT = 8011


def _require_financial_scope(request: Request) -> ServiceIdentity:
    """Depends() para POST /run: exige scope agent:financial:run."""
...
async def lifespan(app: FastAPI) -> AsyncIterator[None]:
    ...
    # Agent + modelo XGBoost treinado uma vez no startup (warm-up)
    agent = FinancialAnalystAgent()
    from agents.financial.tools import _get_model
    _get_model()  # força treino do XGBoost para 1ª request ser rápida
...
def create_app() -> FastAPI:
    ...
    app = FastAPI(
        title="AgroTrust AI – Agent Financial",
        description="Microsserviço HTTP do agente financeiro (Trust Score+ XGBoost).",
        version="0.1.0",
        lifespan=lifespan,
    ...
    @app.post(
        "/run",
        response_model=FinancialOutput,
        tags=["agent"],
        dependencies=[Depends(_require_financial_scope)],
    )
    async def run(input_data: FinancialInput, request: Request) -> FinancialOutput:
        log = logger.bind(
            correlation_id=getattr(request.state, "correlation_id", ""),
            dossie_id=input_data.dossie_id,
        )
        log.info("agent_financial_run_received")
        try:
            agent: FinancialAnalystAgent = request.app.state.agent
            output = await agent.run(input_data)
            log.info(
                "agent_financial_run_completed",
